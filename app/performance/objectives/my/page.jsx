  ...
  const loadData = async () => {
    ...
    try {
      ...
      const objRes = await fetch(
        `/api/performance/objectives?companyId=${data.user.companyId}&employeeId=${data.employee?.employeeId}&scope=my`,
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch("/api/performance/objectives", {
        ...
        body: JSON.stringify({ ...form, companyId: user.companyId, employeeId: employee?.employeeId, scope: "my" }),
      });
      if (res.ok) { setShowAdd(false); setForm({ title: "", description: "", dueDate: "", status: "NOT_STARTED" }); await loadData(); }
  ...
  if (loading) return (
    <div className="app-shell">
      ...
      <div className="app-body">
        <Sidebar activePath="/performance/objectives/my" />
      ...
      <div className="app-body">
        ...
        <main className="main-content">
          ...
          <div className="page-header">
            <div>
              <h1 className="page-title">My Objectives</h1>
              <p className="page-subtitle">Track and manage your personal goals</p>
            </div>
            <button className="btn btn-primary" onClick={() => setShowAdd(true)}>+ Add Objective</button>
          </div>
      ...
      {showAdd && (
        <div className="modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            <h3 className="modal-title" style={{ marginBottom: "1.25rem" }}>New Objective</h3>
            <div className="docs-form-group">
              <label className="docs-form-label">Title</label>
              <input className="form-control" placeholder="e.g. Improve customer satisfaction score" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} autoFocus />
            </div>
            <div className="docs-form-group">
